...
def move_random_direction(amount_max: int, duration_max: int):
    ...
    # TODO: implement a method like this but with vectors instead, and moves relative to previous position, and prevent cursor from getting stuck at edge
    match direction:
        case 'N':
...
def open_paint():
    # search for paint app in search bar and open
...
def draw_circle():
    # almost works
    # TODO: fix this so it doesnt draw a line from the center out to the radius before dragging cursor around circumference
    #   may also need to adjust values for STEP and RADIUS
    STEP = 3
    RADIUS = 400
    X, Y = pyautogui.position()
    for angle_deg in range(0, 360, STEP):
        angle_rad = radians(angle_deg)
        pyautogui.dragTo(X + RADIUS * cos(angle_rad), Y + RADIUS * sin(angle_rad))


def switch_color(current_color: str):
    # swaps between color 1 and color 2
...
def make_less_chaotic_art():
    pass


def main():
    # with these params, program runs for a little over 2 mins
    # start = datetime.now()
    # make_chaotic_art(num_movements=100, duration_max=3, amount_max=80)
    # end = datetime.now()
    # print(f"\nElapsed Time: {str(end - start)[2:]}\n")
    open_paint()
    # pyautogui.moveTo(738, 76)
